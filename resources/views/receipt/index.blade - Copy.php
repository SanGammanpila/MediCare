...
<div class="container">
  ...
  <div class="row">
    ...
    <div class="col shadow-sm p-3 mb-5 bg-white rounded">
      ...
      <datalist id="medicine_hints"></datalist>
      
      <div>
        <input list="medicine_hints" id="new_medicine" type="text" placeholder="Medicine list"  data-list="medicine_hints">
        <datalist id="medicine_hints"></datalist>
...
<hr>
<p>index.blade -copy.php</p>
<button type="button" onclick="loadHistory()" class="btn btn-primary btn-lg btn-block text-info">Load History</button> 
<div id="history">
...
var count =0;
  function addMedicine()
  {
    ...
    var ele_med ='<input id="med' +count+ '" value="'+ med +'" type="hidden" name="med[]">'
  ...
  function receiptView(date,medicines,checkups,notes)
  {
    ...
    var date = '<p class="text-muted text-primary">Date: ' +date+'</p>';
    var medicines_table ='';
    var checkup_table = '';
    var notes_table = '';
    
    if (medicines == undefined || medicines.length == 0)
      medicines_table ='<div class="alert alert-dark" role="alert">No Medicines</div>'
    else{
    medicines_table += '<h5>Medicines</h5>\
                           <table class="table table-striped table-sm">\
                           <thead class="thead-dark"> <tr><th scope="col">#</th><th scope="col">Medicine</th><th scope="col">Dosage</th></tr></thead><tbody>';

    medicines.forEach(function(item,index,array){
     var row = '<tr><td>' +index + '</td>'
      row += '<td>' + item['medicine_id'] +'</td>';
      row += '<td>' + item['dose'] +'</td></tr>'
      medicines_table += row;
    });
...
if (medicines == undefined || medicines.length == 0)
  checkup_table = '<div class="alert alert-dark" role="alert">No Checkups</div>'; 
else
{
  checkup_table += '<h5>Checkups</h5>\
                           <table class="table table-striped table-sm">\
  ...
  checkup_table += '</tbody></table>';
}
if (notes == undefined || notes.length == 0)
notes_table = '<div class="alert alert-dark" role="alert">No Notes</div>'; 
else{
  var notes_table = '<h5>Notes</h5>\
                         <table class="table table-striped table-sm">\
                         <thead class = "thead-dark"> <tr><th scope="col"></th><th scope="col">Note</th><th></th></tr></thead><tbody>';
notes.forEach(function(item,index,array){
    var row = '<tr><td>' +index + '</td>'
    row += '<td>' + item['note_type'] +'</td>';
    row += '<td>' + item['note'] +'</td></tr>';
    notes_table += row;
  });
  notes += '</tbody></table>';
}
